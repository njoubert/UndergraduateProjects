...
public class Semaphore {
	...
	public void run() {
	    for (int i=0; i<10; i++) {
		ping.P(); //Here Ping waits until Pong runs
		Lib.assertTrue(pongs == pings);
		pong.V(); //Here
		pings++;
		TestStubs.debugPrint("Ping: ping=" + pings + " pong=" + pongs);
		Lib.assertTrue(pings == pongs + 1);
	    }
	}
    ...
    public static void selfTest() {
        TestStubs.testStartPrint("Enter Semaphore.selfTest");
	...
	for (int i=0; i<10; i++) {
	    ping.V();  //Enables Ping to wake up
	    pong.P();  //Here Pong waits until ping wakes us up
	    pongs++;
	    TestStubs.debugPrint("Ping: ping=" + pings + " pong=" + pongs);
	    Lib.assertTrue(pongs == pings);
	}
	TestStubs.testDonePrint("Finished Semaphore.selfTest");
    }

    private static int pongs;
