...
public class BinaryTree<Item> {
  ...
  public static void main (String [ ] args) {
    ...
    System.out.println(balancedTree().size());
    Object[] njar = balancedTree().flatten();
    
    //Doing it with an iterator Depth-first
    BinaryTree blah = balancedTree();
    Iterator<TreeNode<String>> blahIt = blah.DepthIterator();
    
    System.out.println("Depth-first iterator:");
    while (blahIt.hasNext()) {
    	System.out.print(blahIt.next().myItem + " ");
    }
    System.out.println();
    
    //Doing it with an interator in-order
    Iterator<TreeNode<String>> blahInOrder = blah.inOrderIterator();
    System.out.println("In-order interator:");
    while(blahInOrder.hasNext()) {
    	System.out.print(blahInOrder.next().myItem + " ");
    }
  }
  ...
  /** Size of a given binaryTree */
  public int size() {
	  return size(myRoot);
  }
  
  private static int size (TreeNode a) {
	  if (a != null) {
  ...
  public Object[] flatten() {
	  ...
	  Iterator<TreeNode<Item>> iter = inOrderIterator();
	  for (int i = 0; iter.hasNext(); i++) {
		  vals[i] = iter.next().myItem;
	  }
	  return vals;
  }
  ...
  public Iterator<TreeNode<Item>> DepthIterator() {
  ...
  public Iterator<TreeNode<Item>> inOrderIterator() {
	  return new InOrderIterator();
  }
  /** Iterates through all my TreeNodes in InOrder. */
  public class InOrderIterator implements Iterator<TreeNode<Item>> {
	    ...
	    public TreeNode<Item> next ( ) {
	      ...
	      if (node.right != null) {
	    	  fringe.push(node.right);
	    	  pushAllLeftiesOnStack(node.right);
	      }
	      return node;
	    }
	    
	    private void pushAllLeftiesOnStack(TreeNode<Item> node) {
