...
-(IBAction)waypointButtonPressed {
	...
	UIAlertView *alert = [[UIAlertView alloc]
						  initWithTitle: @"Waypoint" message:message delegate:nil cancelButtonTitle:@"Done" otherButtonTitles:nil];
	[alert show];
	[alert release];
	[message release];
}
...
-(IBAction)locateButtonPressed {
	...
	if (location.horizontalAccuracy > 0) {
		...
		mySpan.latitudeDelta = (CLLocationDegrees) 0.10;
		mySpan.longitudeDelta = (CLLocationDegrees) 0.10;
		myRegion.span = mySpan;
		[myMap setRegion: myRegion animated: true];
		
	} else {
		
		NSString *message= [[NSString alloc] initWithString: @"We couldn't determine your location! Maybe wait a minute and try again?"];
		UIAlertView *alert = [[UIAlertView alloc]
