...
class Game:
  ...
  def sendNext(self, stateNumber):
    ...
    #thepickle = cPickle.dumps((stateNumber, observation), protocol=cPickle.HIGHEST_PROTOCOL)
...
def saveStats():
  ...
  passwd.close()
  print "Saving output"
  trueStats = open(winFile, "a")
  trueStats.write(len(passwdDict))
  trueStats.write('\n')
  trueStats.close();
  print "Done."
...
def readCommand( argv ):
  ...
  if rest.startswith('++'): launchServerArray(rest[2:])
  elif rest != '': raise Exception('The following could not be interpreted: %s' % rest)
  
  global statsFile, gameConfig, winFile
  ...
  winFile = "win-port-%d.dict" % gameConfig['port']
