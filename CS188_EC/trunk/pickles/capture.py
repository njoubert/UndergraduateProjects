...
def readCommand( argv ):
  ...
  parser.add_option('--replay', dest='gameToReplay', 
                    help=default('A Pickled game to replay'), default=None)
...
def replayGame( gameToReplay, display ):
    rules = CaptureRules()
    finalState = gameToReplay.state
    game = rules.newGame( finalState.data.layout, gameToReplay.agents, display, 10000 )   
    state = game.state
    display.initialize(state.data)
    
    for action in gameToReplay.moveHistory:
      # Execute the action
