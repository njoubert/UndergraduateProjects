...
float calculateEndpointX(float th1, float th2, float th3, int l1, int l2, int l3) {
  float xpos = skel.root.home.x + l1*sin(th1) + l2*sin(th1 + th2) + l3*sin(th1 + th2 + th3);
  return xpos;
}

float calculateEndpointY(float th1, float th2, float th3, int l1, int l2, int l3) {
...
void calculateNewAngles(boolean smallStep, int goalx, int goaly) {
  float newth[] = new float[numJoints];
  
  float errorThresh = 0.0001;
