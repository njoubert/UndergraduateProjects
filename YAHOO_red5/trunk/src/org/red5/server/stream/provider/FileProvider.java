...
import org.red5.io.composite.impl.CompositeFile;
...
public class FileProvider implements IPassive, ISeekableProvider,
    ...
    public FileProvider(IScope scope, URL url, UrlResolver resolver) {
    	this.scope = scope;
    	this.playlistXMLUrl = url;
    ...
    private synchronized void uninit() {
		...
		if (this.reader != null) {
			if (log.isDebugEnabled())
				log.debug("Closing FileProvider's reader since uninit was called.");
			this.reader.close();
			this.reader = null;
		}
	}
    ...
    public synchronized int seek(int ts) {
    	try {
    		if (this.reader == null) 
    			init();
    		return ((ISeekableFile) this.reader).seekToTime(ts);
    	} catch (IOException e) {
    		if (log.isDebugEnabled())
    			log.debug("Seek attempted before reader was initialized, reader init failed.");
    		return ts;
    	}
