...
void createTetrahedron(VectorTriple *u, float y){
float a = sqrt(1*1 - .5*.5);
	float r = .5 * tan(3.1415/6);
	float b = sqrt(a*a - r*r);
	VectorTriple x1(0, y, a/2);
	VectorTriple x2(-.5, y, -a/2);
	VectorTriple x3(.5, y, -a/2);
	VectorTriple x4(0, b+y, -a/2 + r);
	VectorTriple v1(0, 0, 0);
	VectorTriple v2(0, 0, 0);
	VectorTriple v3(0, 0, 0);
	VectorTriple v4(0, 0, 0);
...
void f3(VectorTriple *u){
	...
	//float n1DOTn2 = dot(n1.normalize(), n2.normalize());
	VectorTriple nCross = cross(n1.normalize(), n2.normalize());
	
	float theta2 = asin(dot(nCross, nCross));
	
	VectorTriple Fbend;
	if(nCross.valueX() < 0 || nCross.valueY() < 0 || nCross.valueZ() < 0)
	Fbend = n1*k_b*theta2;
	else if(nCross.valueX() > 0 || nCross.valueY() > 0 || nCross.valueZ() > 0)
	Fbend = n1*-k_b*theta2;
	else
	Fbend = Fnull;
	//VectorTriple Fbend2 = n1*-k_b*theta2;
...
/*	cout<<"n1: "; n1.print();
	...
	VectorTriple CornerForce(0,CF,0);
		...
		u[5] = FlistSum[1]+Fexternal+Fbend;
...
void Euler(VectorTriple *u_tnew, VectorTriple *u_t, float delta_t, void (*fhandle)(VectorTriple*) ){
				//
	VectorTriple u_curr[8];//VectorTriple u_curr[8];
	...
	VectorTriple Fnull(0,0,0);
		for(int i = 0; i < uSize2; i++){
			...
			if(u[i].valueY() < groundPlane){
				u[i].setY(-.5);
				if(u[i+uSize2].valueY() < 0){
					//u[i].print();
					u[i+uSize2].print();
					cout<<endl;
					u[i+uSize2].setY(u[i+uSize2].valueY()*-.5);
					u[i+uSize2].print();
					cout<<endl;
				}
			}	
