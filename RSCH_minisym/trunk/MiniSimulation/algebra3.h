...
//friend mat3 operator / (const mat3& a, const vec3 <vector> & b);
//friend vec3 <vector> operator * (const vec3 <vector> b, const mat3& a);
...
friend mat3 operator - (const vecVec& a);						// -m1
friend mat3 operator + (const vecVec& a, const vecVec& b);	    // m1 + m2
friend mat3 operator - (const vecVec& a, const vecVec& b);	    // m1 - m2
friend mat3 operator * (const vecVec& a, const vecVec& b);		// m1 * m2
friend mat3 operator * (const vecVec& a, const double d);	    // m1 * 3.0
friend mat3 operator * (const double d, const vecVec& a);	    // 3.0 * m1
friend mat3 operator / (const vecVec& a, const double d);	    // m1 / 3.0
friend int operator == (const vecVec& a, const vecVec& b);	    // m1 == m2 ?
friend int operator != (const vecVec& a, const vecVec& b);	    // m1 != m2 ?
...
friend void swap(vecVec& a, vecVec& b);			    // swap m1 & m2
...
//void vec3::print() {cout << n[0] << " " << n[1] << " " << n[2] << " " << endl;}
...
/****************************************************************
*																*
*		    vecVec member functions								*
*																*
****************************************************************/

// CONSTRUCTORS

inline vecVec::vecVec() {}

/*
// ASSIGNMENT OPERATORS
inline vec3& vecVec::operator [] ( int i) {
    //assert(! (i < VX || i > VZ));
	vec3 a;
	if(i == v.size())
		v.push_back(a);
...
inline const vec3& vecVec::operator [] ( int i) const {
    //assert(!(i < VX || i > VZ));
	vec3 a;
	if(i == v.size())
		v.push_back(a);
...
inline vecVec& vecVec::operator = ( const vecVec& m )
{ v[0] = m.v[0]; v[1] = m.v[1]; v[2] = m.v[2]; return *this; }
...
inline vecVec& vecVec::operator *= ( const double d )
{ v[0] *= d; v[1] *= d; v[2] *= d; return *this; }
