...
SparseMatrix::SparseMatrix( int iRows, int iColumns)
{
	//
...
//
bool SparseMatrix::PostMultiply( Physics_LargeVector &V, Physics_LargeVector &Dst )
{
...
//
bool SparseMatrix::PreMultiply( Physics_LargeVector &V, Physics_LargeVector &Dst )
{
...
//
bool SparseMatrix::Add( SparseMatrix &M, SparseMatrix &Dst )
{
...
// Calculate Dst = this - M
//
bool SparseMatrix::Subtract( SparseMatrix &M, SparseMatrix &Dst )
{
	if( (M.m_iColumns != m_iColumns) || (Dst.m_iColumns != m_iColumns) ||
		(M.m_iRows != m_iRows) || (Dst.m_iRows != m_iRows) )
		return false;
...
bool SparseMatrix::Scale( Physics_t scale, SparseMatrix &Dst )
{
	int i, j, ct, idx;
...
Physics_Matrix3x3 & SparseMatrix::operator() (int row, int col)
{
	int i, k, l, ct;
...
void SparseMatrix::Dump( char *szTitle )
{
	char szTemp[100];
	int i,j, k, idx, ct;
