...
public:
   ...
   inline Vector3(const Vector3 &v, Real f) {			    // Copy constructor
      m_afTuple[0] = v.m_afTuple[0]*f;
      m_afTuple[1] = v.m_afTuple[1]*f;
      m_afTuple[2] = v.m_afTuple[2]*f;
   }
   inline Vector3(const Vector3& a, const Vector3& b) {
      m_afTuple[0] = a[0] - b[0];
      m_afTuple[1] = a[1] - b[1];
      m_afTuple[2] = a[2] - b[2];
   }
   inline void Mad(const Vector3 &v, Real f) {
      m_afTuple[0] += f*v.m_afTuple[0];
      m_afTuple[1] += f*v.m_afTuple[1];
      m_afTuple[2] += f*v.m_afTuple[2];
   }
    // construction
...
/*
#ifndef NO_GRAPHICS  
    explicit Vector3 (const Ogre::Vector3 & v)
    {
       m_afTuple[0] = v.x;
       m_afTuple[1] = v.y;
       m_afTuple[2] = v.z;
