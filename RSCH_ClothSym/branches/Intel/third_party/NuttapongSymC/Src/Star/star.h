...
void statisticsquality(struct behavior *behave,struct inputs *in,struct tetcomplex *plex);
typedef struct segment segment;
typedef struct proxipool proxipool;
struct segment *constrainreadrecoversegments(struct behavior *behave,struct inputs *in,FILE *polyfile,struct proxipool *vertexpool,struct outputs *out,struct tetcomplex *plex);
void incrementaldelaunay(struct behavior *behave,struct inputs *in,struct proxipool *vertexpool,struct outputs *out,struct tetcomplex *plex);
void incrementalcheckdelaunay(struct behavior *behave,struct tetcomplex *plex);
typedef size_t starulong;
typedef starulong arraypoolulong;
typedef arraypoolulong tag;
int incrementalfindfirsttet(struct behavior *behave,struct inputs *in,tag *tags,struct proxipool *pool,tag firsttet[4]);
...
enum locateresult locatepoint(struct behavior *behave,struct tetcomplex *plex,tag vertexlocate,tag vertex1,tag vertex2,tag vertex3,tag foundtet[4]);
#define ONETHIRD 0.333333333333333333333333333333333333333333333333333333333333
typedef struct arraypool arraypool;
void bowyerwatson(struct behavior *behave,struct tetcomplex *plex,struct arraypool *trianglelist,tag newvertex,tag vtx1,tag vtx2,tag vtx3,tag vtx4,tag returntriangle[3]);
...
void inputmaketagmap(struct proxipool *vertexpool,starulong firstnumber,tag *vertextags);
...
tag link2dfindringnocache(struct proxipool *pool,tag link,tag groundtag,tag searchvertex);
void link2dcacheinit(link2dcache cache);
void link2dprint(struct proxipool *pool,tag link,tag groundtag);
void link2diterate(struct proxipool *pool,struct link2dposition *pos,tag vertexandlink[2]);
...
void linkringrotateatoms(struct proxipool *pool,tag linkring,struct linkposition *pos1,struct linkposition *pos2,struct linkposition *pos3);
tag linkringdeleteatoms(struct proxipool *pool,struct linkposition *deleteposition,int numatoms);
tag linkringdeleteatoms2(struct proxipool *pool,struct linkposition *deleteposition,int numatoms);
tag linkringinsertatoms(struct proxipool *pool,struct linkposition *insertposition,int numatoms,char *newatombuffer,proxipoolulong allocindex);
tag linkringinsertatoms2(struct proxipool *pool,struct linkposition *insertposition,int numatoms,char *newatombuffer,proxipoolulong allocindex);
void linkringprint(struct proxipool *pool,tag linkring,tag groundtag);
tag linkringiterate(struct proxipool *pool,tag groundtag,struct linkposition *pos);
void linkringiteratorinit(struct proxipool *pool,tag linkring,struct linkposition *pos);
